  ...
  const fetchInstallationOrders = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        "https://sales-order-furniture-server.onrender.com/api/installation-orders",
        {
          headers: {
                ...
                >
                  ...
                  <span style={{ fontSize: "1rem", color: "#555" }}>
                    ...
                    {viewOrder.deliveryDate
                      ? new Date(viewOrder.deliveryDate).toLocaleDateString(
                          "en-GB"
                        )
                      : "N/A"}
                  </span>
                ...
                viewOrder.products.length > 0 ? (
                  viewOrder.products.map((product, index) => (
                    ...
                    >
                      ...
                      {Array.isArray(product.modelNos) &&
                        product.modelNos.length > 0 &&
                        product.modelNos[0].trim() !== "" && (
                          <span style={{ fontSize: "1rem", color: "#555" }}>
                            <strong>Model Nos:</strong> {product.modelNos[0]}
                          </span>
                        )}
                    </div>
                  ))
